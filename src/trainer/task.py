  ...
  def on_epoch_begin(self, epoch, logs={}):
    if epoch > 0 and epoch % self.eval_frequency == 0:
      ...
      if len(checkpoints) > 0:
        checkpoints.sort()
        bh_clone_model = load_model(checkpoints[-1])
        bh_clone_model = model.compile_model(
            bh_clone_model, self.learning_rate)
        loss, acc = bh_clone_model.evaluate_generator(
            model.generator_input(self.eval_files, chunk_size=CHUNK_SIZE),
            steps=self.steps)
        print '\nEvaluation epoch[{}] metrics[{:.2f}, {:.2f}] {}'.format(
            epoch, loss, acc, bh_clone_model.metrics_names)
...
def dispatch(train_files,
             ...
             num_epochs,
             checkpoint_epochs):
  bh_clone_model = model.model_fn(INPUT_SIZE, CLASS_SIZE)
